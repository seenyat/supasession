...
import { AnyMessageSchema, HelloMessageSchema, } from "@supasession/shared";
...
const broadcastToConsumers = (sessionId, message) => {
...
const forwardToProducer = (sessionId, message) => {
...
const handleMessage = (client, raw) => {
    const parseResult = Effect.runSync(Effect.either(S.decodeUnknown(AnyMessageSchema)(JSON.parse(raw))));
...
const handleConnection = (ws, sessionId, autoJoin) => {
    let client = null;
    ws.on("message", (data) => {
        ...
        if (!client) {
            try {
                ...
                const helloResult = Effect.runSync(Effect.either(S.decodeUnknown(HelloMessageSchema)(parsed)));
                ...
                if (client.role === "producer") {
                    ...
                    console.log(`Consumer connected: ${finalSessionId} (${consumers.get(finalSessionId).size} total)`);
                }
                sendMessage(ws, {
                    v: 1,
                    sessionId: finalSessionId,
